...
from mbarc.utils import MeanAttention, ActionInjector, standardize_frame, get_timing_signal_nd, mix, \
    Container, bit_to_int, int_to_bit, ParameterSealer
...
class RewardEstimator(nn.Module):
    ...
    def forward(self, x_mid, x_fin):
        x_mid = torch.mean(x_mid, dim=(2, 3))
        x_fin = torch.mean(x_fin, dim=(2, 3))
        x = torch.cat((x_mid, x_fin), dim=1)
        x = self.dense1(x)
        x = F.relu(x)
        x = self.dense2(x)
        return x
...
class ModifiedRewardEstimator(nn.Module):

    def __init__(self, config, c1, c2, channels=96, dropout=0.3):
        super().__init__()
        self.config = config

        self.embed1 = nn.Conv2d(c1, channels, kernel_size=1)
        self.embed2 = nn.Conv2d(c2, channels, kernel_size=1)

        downscale = []
        for _ in range(self.config.compress_steps):
            downscale.append(nn.Dropout2d(dropout))
            downscale.append(nn.Conv2d(channels, channels, kernel_size=4, stride=2, padding=1))
            downscale.append(nn.ReLU())
            downscale.append(nn.InstanceNorm2d(channels))
        self.downscale = nn.Sequential(*downscale)

        h, w = self.config.frame_shape[1:]
        size = channels * 2 * (h // 2 ** self.config.compress_steps) * (w // 2 ** self.config.compress_steps)
        self.features = nn.Linear(size, 128)

        self.classifier = nn.Linear(128, 3)

    def forward(self, x_mid, x_fin):
        x_mid = self.embed1(x_mid)
        x_fin = self.embed2(x_fin)

        x_fin = self.downscale(x_fin)
...
class NextFramePredictor(Container):
    ...
    def __init__(self, config, n_action):
        ...
        if self.config.use_modified_model:
            reward_estimator = ModifiedRewardEstimator(self.config, middle_shape[0], self.config.hidden_size)
        else:
    ...
    def forward(self, x, action, target=None, epsilon=0):
        ...
        x_fin = x
        reward_pred = self.reward_estimator(x_mid, x_fin)
